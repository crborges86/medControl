...
class MedicationPetEditorScreen extends StatefulWidget {
  final MedicationPet? medication;

  const MedicationPetEditorScreen({super.key, this.medication});

  @override
  State<MedicationPetEditorScreen> createState() => _MedicationPetEditorScreenState();
}

class _MedicationPetEditorScreenState extends State<MedicationPetEditorScreen> {
  ...
  final MedicationPetController _controller = MedicationPetController();
  ...
  Future<void> _saveMedication() async {
    if (_formKey.currentState!.validate()) {
      final medication = MedicationPet(
        ...
        petId: widget.medication?.petId ?? 0,
      ...
      await _controller.updateMedication(medication);
  ...
  Future<void> _archiveMedication() async {
    if (_formKey.currentState?.validate() ?? false) {
      final medication = MedicationPet(
        id: widget.medication?.id,
        petId:
            widget.medication?.petId ?? 0, // Set appropriate profile ID
        name: _nameController.text,
        dosage: _dosageController.text,
        scheduleTimes: _scheduleTimes,
        stock: int.tryParse(_stockController.text) ?? 0,
